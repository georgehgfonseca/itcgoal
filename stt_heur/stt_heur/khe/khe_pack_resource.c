...
static void KheMeetAccumulateGoodness(KHE_MEET meet, int *goodness)
{
  ...
  *goodness += KheMeetDuration(meet) * 20;
  leader_meet = KheMeetLeader(meet, &junk);
  ...
  KHE_RESOURCE_NODE rn, FILE *fp)
{
  ...
  {
    fprintf(stderr, "cost (%.4f > %.4f)", KheCostShow(KheSolnCost(soln)),
...
/*  assigned task group of tasking, one resource node for each resource      */
