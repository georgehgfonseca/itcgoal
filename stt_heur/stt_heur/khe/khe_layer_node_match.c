...
{
  KHE_LAYER layer1;  int res, i;
  res = -1;
  for( i = 0;  i < KheNodeParentLayerCount(node1);  i++ )
  {
    layer1 = KheNodeParentLayer(node1, i);
    if( KheLayerIndex(layer1) > res && !KheLayerContains(layer1, node2) )
      res = KheLayerIndex(layer1);
  }
  return res;
}
  ...
  KHE_SUPPLY_NODE sn, KHE_COST *cost)
{
  ...
  else
  {
    ...
    if( DEBUG4 && i == MArraySize(dn->meets) )
    {
      ...
      fprintf(stderr, "  is %.4f\n", KheCostShow(cost_after));
      /* ***
      fprintf(stderr, "  is %.4f - %.4f\n",
  ...
  int indent, FILE *fp)
{
  if( verbosity >= 1 )
  {
    ...
    if( MArraySize(dn->meets) > 0 )
      ...
      fprintf(fp, "Node %d", KheNodeIndex(dn->node));
    if( indent >= 0 )
      fprintf(fp, "\n");
  }
}
...
static bool KhePartLayerSolve(KHE_PART_LAYER pl)
{
  ...
  if( KheSolnCost(soln) < cost )
  {
    ...
    if( DEBUG3 || DEBUG5 )
      fprintf(stderr, "%c KhePartLayerSolve ret true (pre %.4f, post %.4f)\n",
...
/*****************************************************************************/
/*                                                                           */
/*  void KheLayerNodeMatchingRepairNodeTimes(KHE_NODE parent_node)           */
/*                                                                           */
...
{
  KHE_LAYER_NODE_SOLVER lns;  bool add_layers;
  if( DEBUG1 )
    fprintf(stderr, "[ KheLayerNodeMatchingNodeRepairTimes(Node %d): %.4f\n",
      KheNodeIndex(parent_node),
      KheCostShow(KheSolnCost(KheNodeSoln(parent_node))));
  ...
  if( DEBUG1 )
    fprintf(stderr, "] KheLayerNodeMatchingNodeRepairTimes returning: %.4f\n",
      KheCostShow(KheSolnCost(KheNodeSoln(parent_node))));
}
...
void KheLayerNodeMatchingLayerRepairTimes(KHE_LAYER layer)
{
  KHE_LAYER_NODE_SOLVER lns;
  if( DEBUG1 )
    fprintf(stderr, "[ KheLayerNodeMatchingLayerRepairTimes(Layer %d): %.4f\n",
      KheLayerIndex(layer), KheCostShow(KheSolnCost(KheLayerSoln(layer))));
  ...
  KheLayerNodeSolverDelete(lns);
  if( DEBUG1 )
    fprintf(stderr, "] KheLayerNodeMatchingLayerRepairTimes returning: %.4f\n",
